...
function upload_image($id) {
	...
	$storepath = $upload_base.CUTEGIGS_DIRS."posters".CUTEGIGS_DIRS;
	if(!file_exists($storepath)) {
		mkdir($storepath, 0777);
	} 
	
	$thumbpath = $upload_base.CUTEGIGS_DIRS."posters".CUTEGIGS_DIRS."thumbs".CUTEGIGS_DIRS;
	if(!file_exists($thumbpath)) {
		mkdir($thumbpath, 0777);
	} 
	
	$sidepath = $upload_base.CUTEGIGS_DIRS."posters".CUTEGIGS_DIRS."sides".CUTEGIGS_DIRS;
	...
	if (in_array($file_type, $limited) && ($_FILES[$id]['size'] < 9242880)) {
		if ($_FILES[$id]['error'] > 0) {
			...
			$image = Image::new_from_post($id, 543, 768);
			$image->resize(1,0);
			$image->save_as($imgname, $ext_type, 9);
			
			$imgname = $thumbpath . $file_name;
			$image = Image::new_from_post($id, cutegigs_THUMB_WIDTH, cutegigs_THUMB_HEIGHT);
			$image->resize();
			$image->save_as($imgname, $ext_type, 9);
			
			$imgname = $sidepath . $file_name;
			$image = Image::new_from_post($id, cutegigs_SIDE_WIDTH, cutegigs_SIDE_HEIGHT);
			$image->resize();
			$image->save_as($imgname, $ext_type, 9);
